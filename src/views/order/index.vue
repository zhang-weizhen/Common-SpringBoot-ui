	...
	export default {
		...
		methods: {
			fetchData() {
				...
				useOrderApi().list(this.state.tableData.param).then(response => {
			...
			onSubmit() {
				...
				if(this.state.submitBtn.type === 'edit'){
					useOrderApi().update(this.state.ruleForm).then(response => {
					...
					useOrderApi().add(this.state.ruleForm).then(response => {
						this.state.submitBtn.loading = false;
						this.closeDialog();
						this.fetchData()
					}).catch(() => {
			...
			deleteInfo(row){
				useOrderApi().delete(row).then(response => {
